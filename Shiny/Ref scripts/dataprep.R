...
sales_selected <- sales %>% select("Project_Name", "Transacted_Price", 
                                   ...
                                   "Postal_Code", "Postal_District","latitude","longitude","Time_to_CBD_s","Dist_to_MRT_m")
...
# 7) Filter Away 0 in Longitude and Latitude and Remove "NA"
rent_selected <- rent_selected %>%
  filter(longitude != 0 & latitude != 0)
rent_selected <- na.omit(rent_selected, c("longitude", "latitude"))
...
# 8) Age as of Sales Date: New derived field from “Completion Date” 
sales_selected <- sales_selected %>%
  mutate("Age" = ifelse((Completion_Date == "-"), "NA",
                    ifelse((Completion_Date == "Uncompleted"), "NA",
                        as.numeric(year((as.Date(Sale_Date, "%d-%b-%y")))-as.numeric(Completion_Date)))))
...
# 10) Filter Away 0 in Longitude and Latitude and Remove "NA"
sales_selected <- sales_selected %>%
  filter(longitude != 0 & latitude != 0)
sales_selected <- na.omit(sales_selected, c("longitude", "latitude"))
#sum(is.na(sales_selected$Age))
